...
// @downloadURL  https://github.com/myso-kr/kr.myso.tampermonkey/raw/master/service/com.naver.search-views.analysis.user.js
// @author       Won Choi
...
// @require      https://cdn.jsdelivr.net/npm/kr.myso.tampermonkey@1.0.25/assets/lib/naver-creator-advisor.js
...
GM_App(async function main() {
    ...
    function find_channels(where) {
        const list = [];
        list.push({ where: ['nexearch', 'm'], channels: ['naver_blog', 'naver_post', 'influencer'] });
    ...
    async function update(keyword, start = 1) {
        const nso = get_nso();
        const items = Array.from(document.querySelectorAll('[data-cr-area="rvw*w"], [data-cr-area="rvw*f"], [data-cr-area="rvw*b"], [data-cr-area="blg*a"], ul.keyword_challenge_list [data-space-id] .keyword_box_wrap'));
        ...
        await Promise.map(items, async (item) => {
            ...
            const popularContentsUniq = _.map(_.groupBy(popularContents, 'date'), (items, date)=>{ let base = _.minBy(items, 'rank'); return { date, rank: base.rank, metricValue: _.sumBy(items, 'metricValue') } });
            const popularContentsTotal = _.sumBy(popularContents, 'metricValue');
            const data = { popularContents, popularContentsUniq, popularContentsTotal, popularContentsHead, popularContentsLast };
