...
GM_App(async function main() {
  ...
  `);
    ...
    function voice(text) {
        ...
        voice.timer = setTimeout(()=>alert(voice.error = '작업 시간을 초과하였습니다. 잠시 후 다시 시도해주십시오.'), 1000 * 60 * 30);
    ...
    async function cache_map_array(root, path, mapper) {
        ...
        async function append(value, ...pk) {
            ...
            if(cache.timestamp && moment().diff(cache.timestamp, 'days') < 3) return (!_.find(items, cache.value) && items.push(cache.value), items);
