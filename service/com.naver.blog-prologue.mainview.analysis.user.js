...
// @require      https://tampermonkey.myso.kr/assets/lib/naver-main.js?v=210617
...
GM_App(async function main() {
  ...
  (async function repeat() {
      Toastify({ text: `메인 노출 데이터 가져오는 중...`, }).showToast();
      const items = await NM_searchAll();
      const finds = items.filter(o=>o.url.includes('blog.naver.com') && o.url.includes(`/${blogId}/`));
      if(finds.length) {
          finds.map((item)=>Toastify({ text: `[${item.label}] ${item.url}`, onClick: ()=>window.open(item.url) }).showToast());
      } else {
          Toastify({ text: `메인 노출 데이터 없음`, }).showToast();
      }
